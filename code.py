import mysql.connector as connector from datetime 
import date 
 
global conn, cursor conn= 
connector.connect(host="localhost",database="SMS",user="root",password="Gopika@_0") 
cursor = conn.cursor() 
  def clear():     
    for _ in range(65): 
        print() 
...
#   purpose             : change item details in items table def modify_item(): 
    clear() 
...
#   purpose                 : To display all the items in items tables def item_list(): 
    clear() 
...
#   purpose             : To generate bills  def billing():     clear()     items =[] 
    bill_no = last_bill_no() if bill_no[0]==None: 
        bill_no =1     else: 
        bill_no = bill_no[0]+1  
    ...
    print('Item Id     Item Name   Price  Qty   Amount ')    
     print('-'*100)     total =0     for item in items: 
    ...
    for item in items: 
        ...
        cursor.execute(sql)     wait= input() 
 
#   function      : Date_wise_sell 
#   purpose       : Create a report on date wise sell or sell between two dates  def date_wise_sell(): 
    clear() 
    print('Sell Between Two Dates -- Screen')     
    print('-'*100) 
    start_date = input('Enter start Date (yyyy-mm-dd) :')     
    end_date   = input('Enter End Date (yyyy-mm-dd) :')     sql = 'select * from bills where bill_date between "{}" and 
"{}"'.format(start_date,end_date) cursor.execute(sql) 
records = cursor.fetchall() 
    clear() 
    print('Bill No       Customer Name         Phone No         Bill Date')     
    print('-'*100)     
    for row in records: 
        print('{:10d} {:30s} {:20s} {}'.format(row[0],row[1],row[2],row[3]))     p
        rint('-'*100) 
    print('\n\nPress any key to continue....')     
    wait= input() 
...
# purpose               : display details of any bill def bill_information(): 
 
    bill_no = input('Enter Bill Number :')     
    sql = 'select b.bill_id,b.name,b.phone,b.bill_date,t.item_id,t.qty,i.item_name,i.price from bills b,transaction t,items i \ where b.bill_id = t.bill_id AND t.item_id= i.id AND \ b.bill_id ={};'.format(bill_no)     cursor.execute(sql)      records = cursor.fetchall()     n = cursor.rowcount     clear() 
...
#  purpose          : Function to display amount collected between two dates def amount_collected(): 
    clear() 
    start_date = input('Enter start Date (yyyy-mm-dd) :')     
    ...
    print('-'*100) 
    sql = 'select sum(t.qty*i.price) from bills b,transaction t,items i \ where b.bill_date between "{}" AND "{}" AND b.bill_id = t.bill_id AND \ t.item_id = i.id;'.format(start_date,end_date)     
    cursor.execute(sql)     
...
# purpose            : function to find out bill information def search_bill(): 
    clear() 
    bill_no = input('Enter Bill Number :')    
     ...
     cursor.execute(sql)      
     records = cursor.fetchall()  = cursor.rowcount 
    clear() 
    print("Bill No :",bill_no)     
    print('-'*100)     
    if n<=0: 
...
#  purpose          : Display search menu on the screen def search_menu():     while True:         clear() 
        print('      S E A R C H    M E N U ')         
        print('-'*100)         print('1.   Item Name')         
...
#  purpose          : Display report menu on the screen def report_menu():     while True:         clear() 
